...
class ToastPlugin {
    ...
    createVueComponent(options) {
        ...
        Object.assign(
            this.vueComponent,
            this.Vue.prototype[this.options.property].options,
            options
        );
    ...
    showToast(message, options = {}) {
        ...
        if (this.vueComponent) {
            // if queue enabled
            if (
                options.queue !== undefined ? options.queue : this.options.queue
            ) {
    ...
    install(Vue, options = {}) {
        ...
        let vueInstanceMethod = this.showToast.bind(this),
            shortcutMethods = [
                {
                    name: "success",
                    options: { color: "success", icon: "mdi-check" }
                },
                {
                    name: "info",
                    options: { color: "info", icon: "mdi-information" }
                },
                {
                    name: "error",
                    options: { color: "error", icon: "mdi-alert-circle" }
                },
                {
                    name: "warning",
                    options: { color: "warning", icon: "mdi-alert" }
                }
            ];
