<template>
    <v-card :elevation="blank ? 10 : 0" class="ma-2" :loading="loading">
        ...
        <v-btn
            :disabled="loading"
            class="btn-change-cover"
            @click="$refs.inputCoverUpload.click()"
        >
        ...
        <v-card-actions>
            ...
            <template v-if="blank">
                <v-btn depressed @click="cancel">
                    <v-icon left :disabled="loading">mdi-cancel</v-icon>
                    Cancel
                </v-btn>
                <v-btn depressed color="info" @click="save">
                    <v-icon left :disabled="loading">mdi-upload-outline</v-icon>
                    Upload
                </v-btn>
            </template>
            <template v-else-if="photo.deleted">
                <v-btn depressed color="error" @click="remove">
                    <v-icon left :disabled="loading">mdi-delete-outline</v-icon>
                    Delete
                </v-btn>
                <v-btn depressed color="success" @click="restore">
                    <v-icon left :disabled="loading">mdi-undo-variant</v-icon>
                    Restore
                </v-btn>
            </template>
            <template v-else>
                ...
                <v-btn depressed color="success" @click="save">
                    <v-icon left :disabled="loading">mdi-check</v-icon>
                    Save
                </v-btn>
            </template>
        </v-card-actions>
...
export default {
    ...
    data() {
        ...
        return {
            loading: false,
            newPhotoSrc: this.blank
                ? ""
                : this.thumbnailSrc(this.photo.src) || "",
        ...
        thumbnailSrc(src) {
            try {
                let lastDotIndex = src.lastIndexOf("."),
                    ...
                    suffix =
                        process.env.VUE_APP_GALLERY_DASHBOARD_THUMBNAIL_SUFFIX;
